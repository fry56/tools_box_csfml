...
bool sprite_is_mouse_click(sprite *self, sfEvent *event_datas)
{
    ...
    if (self->pos.x > (float)event_datas->mouseButton.x || (self->pos.x + sprite_bounds.width) < (float)event_datas->mouseButton.x)
        return false;
    if (self->pos.y > (float)event_datas->mouseButton.y || (self->pos.y + sprite_bounds.height) < (float)event_datas->mouseButton.y)
        return false;
    return true;
}
